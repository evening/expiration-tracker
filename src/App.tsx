...
function App() {
  ...
  const [newExpiration, setNewExpiration] = React.useState<Date>(new Date());
  ...
  const addEntry = (foodName: string, expiration: Date) => {
    setEntries([...entries, { foodName: foodName, expiration: expiration ? expiration : defaultDate }]);
  };

  const removeEntry = (food: Entry) => {
  ...
  return (
    <div className="App">
      ...
      <ul>
        {entries.filter(entry => { if (searchTerm) { return entry.foodName.includes(searchTerm) } return entry })
          .map((food, index) =>
            <li key={index}>
              <>
                {food.foodName} | {food.expiration ? new Date(food?.expiration).toLocaleDateString() : ''}
                <small>
                  <EditFood
                    food={food}
                    index={index}
